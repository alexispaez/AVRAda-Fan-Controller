...
package body MCP9808.I2C is
   ...
   procedure Get_Ambient_Temperature
      ...
      Temperature_Data     : Ambient_Temperature_Register with
        Address => Temperature_Data_Raw'Address;
      Temperature_Raw      : aliased Unsigned_16;
      Temperature          : Ambient_Temperature with
        Address => Temperature_Raw'Address;
   begin
      ...
      if Temperature_Data.T_SIGN = False then
         ...
         -- Calculate using < 0ºC formula
         Temperature_Raw := 4_096 - Unsigned_16 (Temperature_Data.T_Buffer);
      end if;
   ...
   procedure Get_Resolution
     (This       : in out MCP9808_Temperature_Sensor_I2C;
      Resolution :    out Resolution_Bits; Status : out Boolean)
   is
      Resolution_Data_Raw : Unsigned_8;
      Resolution_Data     : Resolution_Register with
        Address => Resolution_Data_Raw'Address;
   begin
      -- Get the raw resolution data from the sensor
      AVR.Interrupts.Enable;
      Master.Send_And_Receive
        (This.I2C_Address, Unsigned_8 (RESOLUTION_REG), Resolution_Data_Raw);
